...
namespace CourseTech.Application.Services
{
    public class StatisticService : IStatisticService
    {
        ...
        private readonly IAnswerAnalysisRepository _answerAnalysisRepository;
        ...
        public StatisticService(
            ...
            IAnswerAnalysisRepository answerAnalysisRepository)
        {
        ...
        public async Task<DataResult<UserStatisticsDto>> GetUserStatisticsAsync(Guid userId)
        {
            try
            {
                var user = await _userRepository.GetByIdAsync(userId);
                if (user == null)
                    return DataResult<UserStatisticsDto>.Failure("Пользователь не найден");
                ...
                return DataResult<UserStatisticsDto>.Success(statistics);
            }
            catch (Exception ex)
            {
                return DataResult<UserStatisticsDto>.Failure($"Ошибка при получении статистики: {ex.Message}");
            }
        }

        public async Task<CollectionResult<CategoryProgressDto>> GetCategoryProgressAsync(Guid userId)
        {
            try
            {
                ...
                return CollectionResult<CategoryProgressDto>.Success(progressList);
            }
            catch (Exception ex)
            {
                return CollectionResult<CategoryProgressDto>.Failure($"Ошибка при получении прогресса: {ex.Message}");
            }
        }

        public async Task<DataResult<SessionSummaryDto>> GetSessionSummaryAsync(long sessionId, Guid userId)
        {
            try
            {
                var session = await _sessionRepository.GetByIdAsync(sessionId);
                if (session == null || session.UserId != userId)
                    return DataResult<SessionSummaryDto>.Failure("Сессия не найдена");
                ...
                return DataResult<SessionSummaryDto>.Success(summary);
            }
            catch (Exception ex)
            {
                return DataResult<SessionSummaryDto>.Failure($"Ошибка при получении сводки: {ex.Message}");
            }
        }
